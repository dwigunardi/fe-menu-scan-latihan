...
export default function AdminPoliciesSettingsPage() {
  return (
    <RoleGuard allowedRoles={[ROLE.ADMIN]}>
      <AdminPoliciesSettingsContent />
    </RoleGuard>
  );
}

function AdminPoliciesSettingsContent() {
    ...
    <div className="flex flex-col min-h-screen">
      ...
      <div className="flex-1 p-4 sm:p-6 lg:p-8 max-w-7xl w-full mx-auto space-y-6">
          ...
          <div className="space-y-1">
            <div className="flex items-center gap-2">
              ...
              <h1 className="text-xl sm:text-2xl font-bold tracking-tight text-stone-900 dark:text-zinc-50">
                Kebijakan Operasional Toko & Presensi
              </h1>
            </div>
            <p className="text-xs text-stone-500 dark:text-zinc-400">
              Atur mode operasional toko (Shift-Driven/Clock-Driven), toleransi menit keterlambatan, dan status darurat.
            </p>
          </div>
        </div>
        ...
        {isLoading ? (
          ...
          <StorePoliciesForm initialData={setting} />
        )}
      </div>
    </div>
  );
